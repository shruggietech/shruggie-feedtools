...
class TestWPRestAdapter:
    """WordPress REST API adapter tests."""

    def test_wp_rest_extracts_title(self, load_fixture):
        content = load_fixture("wp_rest/posts_embedded.json")
        result = parse_wp_rest(content)
        for item in result["items"]:
            assert item.get("title"), "Item should have a title"

    def test_wp_rest_extracts_content(self, load_fixture):
        content = load_fixture("wp_rest/posts_embedded.json")
        result = parse_wp_rest(content)
        has_content = any(item.get("content") for item in result["items"])
        assert has_content

    def test_wp_rest_extracts_excerpt(self, load_fixture):
        content = load_fixture("wp_rest/posts_embedded.json")
        result = parse_wp_rest(content)
        has_desc = any(item.get("description") for item in result["items"])
        assert has_desc

    def test_wp_rest_embedded_author(self, load_fixture):
        content = load_fixture("wp_rest/posts_embedded.json")
        result = parse_wp_rest(content)
        has_author = any(item.get("author") for item in result["items"])
        assert has_author

    def test_wp_rest_embedded_featured_media(self, load_fixture):
        content = load_fixture("wp_rest/posts_embedded.json")
        result = parse_wp_rest(content)
        has_thumb = any(item.get("thumbnail") for item in result["items"])
        assert has_thumb

    def test_wp_rest_embedded_categories(self, load_fixture):
        content = load_fixture("wp_rest/posts_embedded.json")
        result = parse_wp_rest(content)
        has_cats = any(
            len(item.get("categories", [])) > 0 for item in result["items"]
    ...
    def test_wp_rest_guid(self, load_fixture):
        content = load_fixture("wp_rest/posts_embedded.json")
        result = parse_wp_rest(content)
        for item in result["items"]:
            assert item.get("guid"), "Item should have a guid"

    def test_wp_rest_dates_gmt(self, load_fixture):
        content = load_fixture("wp_rest/posts_embedded.json")
        result = parse_wp_rest(content)
        for item in result["items"]:
            pub = item.get("pub_date", "")
            if pub:
                assert pub.endswith("Z"), f"pub_date should end with Z: {pub}"

    def test_wp_rest_base_url_extraction(self, load_fixture):
        content = load_fixture("wp_rest/posts_embedded.json")
        base = "https://example.com/wp-json/wp/v2/posts?_embed"
        result = parse_wp_rest(content, base_url=base)
...
class TestJSONFeedAdapter:
    """JSON Feed adapter tests."""

    def test_json_feed_extracts_title(self, load_fixture):
        content = load_fixture("json_feed/v1_standard.json")
        result = parse_json_feed(content)
        assert result["feed"]["title"]

    def test_json_feed_home_page_url(self, load_fixture):
        content = load_fixture("json_feed/v1_standard.json")
        result = parse_json_feed(content)
        assert result["feed"]["link"]

    def test_json_feed_authors_array(self, load_fixture):
        content = load_fixture("json_feed/v1_standard.json")
        result = parse_json_feed(content)
        assert result["feed"].get("author")

    def test_json_feed_item_content_html(self, load_fixture):
        content = load_fixture("json_feed/v1_standard.json")
        result = parse_json_feed(content)
        has_content = any(item.get("content") for item in result["items"])
        assert has_content

    def test_json_feed_item_tags(self, load_fixture):
        content = load_fixture("json_feed/v1_standard.json")
        result = parse_json_feed(content)
        has_tags = any(
            len(item.get("tags", [])) > 0 for item in result["items"]
        )
        assert has_tags

    def test_json_feed_item_attachments(self, load_fixture):
        content = load_fixture("json_feed/v1_standard.json")
        result = parse_json_feed(content)
        has_enc = any(
            len(item.get("enclosures", [])) > 0 for item in result["items"]
    ...
    def test_json_feed_item_dates(self, load_fixture):
        content = load_fixture("json_feed/v1_standard.json")
        result = parse_json_feed(content)
        has_pub = any(item.get("pub_date") for item in result["items"])
        assert has_pub

    def test_json_feed_item_image(self, load_fixture):
        content = load_fixture("json_feed/v1_standard.json")
        result = parse_json_feed(content)
        has_thumb = any(item.get("thumbnail") for item in result["items"])
        assert has_thumb

    def test_json_feed_item_summary(self, load_fixture):
        content = load_fixture("json_feed/v1_standard.json")
        result = parse_json_feed(content)
        has_summary = any(item.get("summary") for item in result["items"])
        assert has_summary
